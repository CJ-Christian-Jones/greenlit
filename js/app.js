...
projectPage = function projectPageV23() {
  ...
  shell(`<div class="arcadePageHead"><div><div class="mini">Step 1</div><h1>Build a ${kind === "show" ? "show" : "movie"}.</h1><p class="sub">Choose the format, reference title, creative direction and fit rules. Then the arcade loop takes over.</p></div><div class="arcadeStepPill">Format first</div></div>
  <section class="v23SetupGrid">
    <div class="v23ChoicePanel"><label>Project format</label><div class="v23Segment"><button data-kind="movie" class="${kind === "movie" ? "active" : ""}">🎬 Movie</button><button data-kind="show" class="${kind === "show" ? "active" : ""}">📺 Show</button></div></div>
    <div class="v23ChoicePanel"><label>Creative direction</label><select id="creativeDirection">${Object.keys(
      ARCADE_DIRECTIONS,
    )
      .map(
        (x) =>
      ...
      .join(
        "",
      )}</select><small>${S.arcade.creativeDirection === "Faithful" ? "Fit matters most." : S.arcade.creativeDirection === "Bold Reinvention" ? "Originality is rewarded." : "Balanced freedom and familiarity."}</small></div>
    <div class="v23ChoicePanel"><label>Fit mode</label><div class="v23Segment"><button data-fitmode="Arcade" class="${S.arcade.fitMode === "Arcade" ? "active" : ""}">Arcade</button><button data-fitmode="Realistic" class="${S.arcade.fitMode === "Realistic" ? "active" : ""}">Realistic</button></div><small>${S.arcade.fitMode === "Realistic" ? "Age, gender and medium strongly affect fit. Body type is never used." : "Age and gender are soft suggestions."}</small></div>
  </section>
  <section class="arcadeSearchPanel"><div class="arcadeSearchTabs"><div class="arcadeTitleSearch"><input id="titleSearch" placeholder="Search a ${kind === "show" ? "show" : "movie"} title…"><button class="btn primary" id="searchTitleBtn">Search</button></div><button class="btn warn" id="randomTitleBtn">🎲 Randomize</button></div><div class="arcadeMovieFilters"><div><label>Genre</label><select id="movieGenre"><option value="">Any genre</option>${GENRES.map((g) => `<option value="${g}">${g}</option>`).join("")}</select></div><div><label>From year</label><input id="movieYearFrom" type="number" min="1930" max="${maxYear}" placeholder="1980"></div><div><label>To year</label><input id="movieYearTo" type="number" min="1930" max="${maxYear}" placeholder="${maxYear}"></div><button class="btn" id="browseMovies">Browse</button></div><div id="searchList" class="arcadeMovieResults"></div></section>
  ${S.reference ? `<section class="arcadePickedMovie">${S.reference.poster ? `<img src="${S.reference.poster}" alt="">` : '<div class="arcadePosterFallback">GL</div>'}<div><div class="mini">Reference ${kind}</div><h2>${arcadeEsc(S.reference.title)}</h2><p>${arcadeEsc(S.reference.overview || "")}</p><div class="chipRow"><span class="chip active">${arcadeEsc(S.reference.genre)}</span><span class="chip">${S.reference.year}</span><span class="chip">${arcadeEsc(S.arcade.creativeDirection)}</span><span class="chip">${arcadeEsc(S.arcade.fitMode)} fit</span></div></div></section>${kind === "show" ? `<section class="v23ShowSettings"><div><label>Episodes</label><input id="episodeCount" type="range" min="4" max="22" value="${S.arcade.episodeCount}"><b>${S.arcade.episodeCount}</b></div><div><label>Episode length</label><input id="episodeLength" type="range" min="20" max="70" step="5" value="${S.arcade.episodeLength}"><b>${S.arcade.episodeLength} min</b></div><div><label>Release</label><select id="releaseModel"><option ${S.arcade.releaseModel === "Weekly" ? "selected" : ""}>Weekly</option><option ${S.arcade.releaseModel === "Binge" ? "selected" : ""}>Binge</option></select></div></section>` : ""}<button class="btn primary arcadeContinue" id="continue">Start casting →</button>` : '<div class="callout">Pick a reference title to begin.</div>'}`);

  $$("[data-kind]").forEach(
    (b) =>
      (b.onclick = () => {
        S.arcade.projectKind = b.dataset.kind;
        S.reference = null;
        S.referenceCredits = null;
        S.arcade.originalCast = [];
        render();
      }),
  );
  ...
  const dateField = kind === "show" ? "first_air_date" : "primary_release_date";
  const showResults = async (loader) => {
    ...
    try {
      ...
      box.innerHTML =
        rows
          .slice(0, 12)
          .map((x) => arcadeMovieResultButton({ ...x, media_type: endpoint }))
  ...
  $("#browseMovies").onclick = () =>
    showResults(async () => {
      const p = { include_adult: "false", sort_by: "popularity.desc", page: 1 };
      const g = $("#movieGenre").value;
      if (g) p.with_genres = GENRE_IDS[g];
      const a = +$("#movieYearFrom").value,
        z = +$("#movieYearTo").value;
      if (a) p[`${dateField}.gte`] = `${a}-01-01`;
      if (z) p[`${dateField}.lte`] = `${z}-12-31`;
      return (await api(`/discover/${endpoint}`, p)).results || [];
    });
  $("#randomTitleBtn").onclick = async () => {
    try {
      const p = {
      ...
      const g = $("#movieGenre").value;
      if (g) p.with_genres = GENRE_IDS[g];
      const rows = (await api(`/discover/${endpoint}`, p)).results || [];
      if (!rows.length) throw new Error("No title matched.");
      await selectReference(
        rows[Math.floor(Math.random() * rows.length)].id,
        endpoint,
      );
      S.arcade.projectKind = kind;
    } catch (e) {
      alert(e.message);
    }
  };
  if ($("#episodeCount"))
    $("#episodeCount").oninput = (e) => {
      S.arcade.episodeCount = +e.target.value;
      e.target.nextElementSibling.textContent = e.target.value;
    };
  if ($("#episodeLength"))
    $("#episodeLength").oninput = (e) => {
      S.arcade.episodeLength = +e.target.value;
      e.target.nextElementSibling.textContent = `${e.target.value} min`;
    };
  if ($("#releaseModel"))
    $("#releaseModel").onchange = (e) =>
      (S.arcade.releaseModel = e.target.value);
  if ($("#continue"))
    $("#continue").onclick = () => {
      S.screen = 1;
...
hirePage = async function v25HirePage() {
  ...
  if (pool?.people?.length)
    await Promise.all(pool.people.map((p) => v25EnrichCareerEra(p, slot)));
  await V25_HIRE_PAGE_BASE();
  $$("[data-short]").forEach((button) => {
    const old = button.onclick;
    button.onclick = (e) => {
      const was = (S.shortlists[slot] || []).includes(button.dataset.short);
      old?.(e);
      if (!was) v25RememberSavedSource(slot, button.dataset.short);
    };
  });
  $$("[data-saved-open]").forEach(
    (button) =>
      (button.onclick = (e) => {
        if (e.target.closest("[data-short-remove]")) return;
        const id = button.dataset.savedOpen;
        const source = S.arcade.savedPickSources?.[slot]?.[id];
        if (Number.isFinite(source)) S.sourceIndex[slot] = source;
